<?php
	...
	if(!empty($formData[0]['value'])){
		...
		if($br_insert->execute()){
			echo '<p class="alert alert-success">New branch has been successfully created</p>';
		}else{
			echo '<p class="alert alert-success">New branch created failed!</p>';
		}

	}else{
		echo '<p class="alert alert-success">Please insert</p>';
	}
